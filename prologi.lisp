...
(defpackage :prolog
  (:use :common-lisp)
  (:export #:clause-head #:clause-body #:get-clauses #:predicate #:*db-predicates* 
           #:clear-db #:clear-predicate #:rename-variables #:?- #:prove-all #:prove
           #:continue-p #:variables-in #:non-anon-variable-p #:replace-?-vars)
  )
...
#+:never (defun args (x) "The arguments of a relation" (rest x)) ; see prolog.c
...
(defun continue-p ()
  ...
  (case (read-char)
    ...
    (#\newline (continue-p))
    (otherwise 
      (format t " Type ';' to see more, or type '.' to stop~%")
      (continue-p))))
