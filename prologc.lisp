...
(defpackage :prolog
  (:use :common-lisp)
  (:export #:args #:<- #:add-clause #:top-level-prove #:+unbound+ #:*var-counter* #:var #:var-p
           #:? #:bound-p #:deref #:unify! #:set-binding! #:undo-bindings! #:prolog-compile 
           #:clauses-with-arity #:relation-arity #:make-predicate #:make-= #:compile-call
           #:prolog-compiler-macro #:def-prolog-compiler-macro #:compile-arg #:has-variable-p
           #:maybe-add-undo-bindings #:bind-unbound-vars #:make-anonymous #:anonymous-variables-in
           #:anon-vars-in #:compile-unify #:compile-if #:compile-unify-variable #:bind-variables-in
           #:follow-binding #:compile-arg #:bind-new-variables 
           #:= #:compile-clause #:*uncompiled* #:run-prolog #:prolog-compile-symbols 
           #:deref-exp #:*predicate* #:compile-predicate #:compile-body 
           *trail*
           #:top-level-query/0 #:show-prolog-vars/2)
  )
...
(defun make-predicate (symbol arity)
  ...
  (intern (format nil "~A/~A" symbol arity))) ; seiji
  ...
  (let ((vars (delete '? (variables-in goals))))
    (add-clause `((top-level-query)
                  ...
                  (show-prolog-vars ,(mapcar #'symbol-name vars)
                                    ,vars))))
  ;; Now run it
  (run-prolog 'top-level-query/0 #'ignores)
  (format t "~&End of prove.")
  (values))
